...
requires_base = ["rengu >= 6.0", "jsonpath-ng"]
requires_extra = {}

requires_extra["all"] = [m for v in requires_extra.values() for m in v]

setup(
    name="rengu-filter-jsonpath",
    version="6.0",
    description="Rengu jsonpath filter",
    ...
    entry_points={
        "rengu_filter": ["jsonpath =  rengu_filter_jsonpath:RenguFilterJsonPath"]
    },
)
